---
title: "Bayesian Statistics Workbook - Chapter 2 Bayes’ Rule"
format:
  pdf:
...
a2_11 <- dbinom(4, 6, 0.3)
n <- 6
pi <- c(0.3, 0.4, 0.5)
f_pi <- c(0.25, 0.6, 0.15)

table <- data.frame(
  "pi" = rep(pi, each = n),
  "f_pi" = rep(f_pi, each = n),
  "y" = rep(1:n,length(unique(pi)))
)

table <- table |> 
  mutate(L_pi_y = dbinom(y,n,pi)) |> 
  mutate(const = L_pi_y*f_pi)
...
f_pi_y <- function(x){
  n_c <- f_y(x)
  post <- table |> 
    mutate(nor_con = rep(n_c, nrow(table))) |> 
    filter(y == x) |> 
    mutate(posterior = f_pi*L_pi_y/n_c) |> 
    select(posterior)
  return(post)
}
...
prior_post_kable <- kable(prior_post)

prior_post <- prior_post |> 
  pivot_longer(cols = -pi,
  names_to = "model",
  values_to = "f_pi")

plot <- ggplot(prior_post, aes(x = pi, y = f_pi, fill = model)) +
  geom_bar(stat = "identity", width = 0.01, position = position_dodge(width = 0.02))+
  geom_text(aes(label = round(f_pi, 3), vjust = -0.5), position = position_dodge(width = 0.02), size = 1.5) +
  scale_y_continuous(limits = c(0, 1)) +
  theme_bw()
```

a) $f(y|\pi)$ would be a binomal model because:

- It has a fixed number of trials, $n=6$.

- The outcome of each trial is only "success" or "fail".

- The probability for success each trial is equal across trials, which is $\pi$.

The pmf for $f(y|\pi)$ is:

$$
f(y|\pi) = \binom{6}{y}\pi^2 \text{ for } y \in \{0,1,2,\dots,6\}
$$

b) Muhammad was offered four of the six internships, which means $y=4$. 

With $\pi=0.3$, we have:

$$
f(4|0.3) = \binom{6}{4}\times 0.3^2
$$

The results is `{r} round(a2_11,3)`.

c) The posterior model of𝜋in light of Muhammad’s internship news is denoted as $f(\pi|y=4)$.
...
where, by the Law of Total Probability, the overall probability of observing data $Y=y$ accrossall possible $\pi$, $f(y)$, is:

$$
f(y=4) = \sum_{\text{all } y}f(\pi)L(\pi|y = 4)
$$

With the Muhammad’s internship news, we can construct the posterior model of $\pi$. The results are:
...
Miles is learning how to make a mug in his ceramics class. A difficult part of the process is creating or “pulling” the handle. His prior model of $\pi$, the probability that one of his handles will actually be good enough for a mug, is below:

| $\pi$    | 0.1  | 0.25  | 0.4  | Total |
|----------|------|------|------|-------|
| $f(\pi)$ | 0.2 | 0.45 | 0.35 | 1     |

a) Miles has enough clay for 7 handles. Let $Y$ be the number of handles that will be good enough for a mug. Specify the model for the dependence of $Y$ on $\pi$ and the corresponding pmf, $f(y|\pi)$.

b) Miles pulls 7 handles and only 1 of them is good enough for a mug. What is the posterior pmf of $\pi$, $f(\pi|y=1)$

c) Compare the posterior model to the prior model of $\pi$. How would you characterize the differences between them?

d) Miles’ instructor Kris had a different prior for his ability to pull a handle (below). Find Kris’s posterior $f(\pi|y=1)$ and compare it to Miles’.

| $\pi$    | 0.1  | 0.25  | 0.4  | Total |
|----------|------|------|------|-------|
| $f(\pi)$ | 0.15 | 0.15 | 0.70 | 1     |
...
- It has a fixed number of trials, $n=7$.
...
$$
f(y|\pi) = \binom{7}{y}\pi^2 \text{ for } y \in \{0,1,2,\dots,7\}
$$

c) The posterior model of pmf of $\pi$ given new information is denoted as $f(\pi|y=1)$.
...
$$
f(y=1) = \sum_{\text{all } y}f(\pi)L(\pi|y = 1)
$$
...
prior_post_kable <- kable(prior_post, digits = 3)
...
c) The posterior distribution reflects the information from the observation, which suggests that a moderate probability for a good handle is more likely, and the belief in very high or very low probabilities has adjusted accordingly.
